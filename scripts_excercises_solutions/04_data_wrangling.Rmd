...
Another way is to use `%in%` (at first glance, it's easily confused with the pipe).
...
Note that there is no output when I assign something to a variable, so I would have to call the variable `nerd_red` again on order to display it.[^display_parentheses]
...
When using `pull()`, what we get back is a vector. When using select, what we get back is a data frame with one column. However, if we only have a single column anyways, a vector is more comfortable to handle and more "lightweight" (The vector is acutally smaller than the data frame containing the same information - check out `object.size(nerd_red["age"])` vs. `object.size(nerd_red$age)`.
PS: As opposed to `nerd_red[["age"]]`, which returns a vector, `nerd_red["age"]` gives back a one-column data frame.
...
Before, we first sorted by age and then we sorted by gender within the age groups. Now it's the other way aroud.
