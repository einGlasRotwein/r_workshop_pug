...
```{r}
hotdog_genders <- 
  c("male", "female", "male", "male", "male", "female", "female",
...
4.1) Can you guess what the second argument in `head()` (and `tail()`) does?
...
5.4) There is also another way. Remember what `length(hotdog_names)` returns. How can you use that for indexing?[^faster]

```{r}
hotdog_names[length(hotdog_names)]
```

[^faster]: By the way, using `tail(hotdog_names, 1)` seems more elegant, but `hotdog_names[length(hotdog_names)]` is much faster. I used the `R`-package `microbenchmark` to compare how fast both versions are and the man run time of `tail(hotdog_names, 1)` is around 18 times slower than the version with `length()`. In practice, this hardly matters because both functions only take nanoseconds to run. However, should you ever find yourself in a situation where speed matters (e.g. when you're running millions of simulations), it is good to know how to keep your code "lightweight".
...
```{r}
hotdog_names_female <- hotdog_names[hotdog_genders == "female"]
hotdog_2019_female <-  hotdog_2019[hotdog_genders == "female"]

hotdog_names_female[hotdog_2019_female == max(hotdog_2019_female)]
```

In one line of code (I added a line break so the code becomes more readable):

```{r}
hotdog_names[hotdog_genders == "female"][
  hotdog_2019[hotdog_genders == "female"] == max(hotdog_2019[hotdog_genders == "female"])
]
```
