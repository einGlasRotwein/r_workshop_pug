...
```{r}
# check whether the age column in is a vector
is.vector(nerd$age)
...
1.1) Select the column `gender` from the `nerd` data frame and save it into a variable. Try out different approaches.
...
1.6) Use the "mess around data frame" to overwrite the entire 1st column. We will solve this task together in a second, but first think about the following questions: How would you, in principle, overwrite the entire 1st column? What would you need to replace it with?

The first column of `mess_around` is a vector, so we have to overwrite it with another vector (of the same length).
...
Note that I only use a single-element vector (the word "cake") to overwrite the entire column.
Because of `R`'s vectorisation, it automatically repeats `"cake"` 1,000 times to adapt it to the length of the vector it has to fill.
If we had used `c("cake", "coffee")` to fill the vector, our small replacement vector would have been repeated 500 times.
The process only fails when the number of elements in the replacement vector is not a multiple of the vector it is supposed to replace, resulting in the very famous error message:

```{r error=TRUE}
mess_around[, 1] <- c("cake", "coffee", "biscuit")
```
...
This is a little refresher of these functions.
...
There seems to be at least one very implausible value here.
...
Summary works only on numeric input. For characters like in the `country`, it can't tell us much more than some very basic information about the column.

2.3) What we want for columns like this is `table()`. Try it on the `country` column.
...
We get a summary for each column - a little bit overwhelming in this case.

Tip: The package `skimr` is a very nice gimmick that produces very elegant summaries. If you want to give it a spin, you have to install it first (run the line that is currently commented out).
...
# You can use any function from a package with this notation.
# We won't go into the details of using packages here.
skimr::skim(nerd)
```

Because `nerd` is quite large, the summary is still overwhelming, but still much easier to digest than the classic `summary()` output. Also note that `skim()` gives you a tiny histrogram for each column, giving you a very simple first overview about the distribution of your variables.

2.5) Try `table()` on the gender column. Does this help? Can you find more information in the codebook? (Folder: skripts_exercises $\to$ data_sets $\to$ nerd_codebook.txt) By the way, you can just open the codebook from within RStudio!

```{r}
table(nerd$gender)
```

Without a codebook, we wouldn't know what these numbers represent. Thanks to the codebook, we know that 1 = Male, 2 = Female, 3 = Other.
