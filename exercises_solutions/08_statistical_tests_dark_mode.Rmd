...
1.1) Quick reminder: Calculate the mean and the standard deviation of the nerd score for each gender and compare the group means with the output of the t-test.

```{r}
nerd_fm %>% 
  group_by(gender) %>% 
  summarise(
    mean_nerd = mean(nerd_score),
...
ttest_gender <- t.test(nerd_fm$nerd_score ~ nerd_fm$gender)
```

```{r}
ttest_gender$p.value
```

2.2) This allows you to print your results right in your R Markdown document, e.g.: The p-value was `r round(ttest_gender$p.value, 3)`.
...
```{r}
t.test(nerd_fm$nerd_score ~ nerd_fm$gender)$p.value
```
...
```{r}
effectsize_gender <- effectsize::cohens_d(nerd_fm$nerd_score ~ nerd_fm$gender)
...
```{r}
nerd_fm %>% 
  rstatix::cohens_d(nerd_score ~ gender)
```
