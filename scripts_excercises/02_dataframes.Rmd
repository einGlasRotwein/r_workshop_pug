...
If we look into our Environment tab, it says that nerd has 15,212 observations (i.e. rows) of 1 variable - i.e. one column.
...
2.2) Use `summary()` on the `major` column (which contains the participants' major during university). What is different here? Why? (Tip: Inspect `nerd$major`, maybe using `head()`).

2.3) Try `table()` for the `major` column. What is the difference?
...
However, it is often most helpful to actually **see** the data, i.e. visualising it.
Some teaching concepts put visualisation first, before doing anything else with the data, and I am a fan of that.
So, get ready for your first plots!

For this, we will install the package `tidyverse`.
...
This is why we will also use both "tidy coding" and "base coding", but feel free to pick whatever feels most comfortable for you.

Let's install the `tidyverse` together.
You already installed some packages during the installation guide, so the process should seem a bit familiar.
...
Before we dive in, let's look at the key differences between base `R` and the `tidyverse`.
Some of them will become apparently throughout this course.
