...
1.4) I want to continue working with a reduced version of our data that only contains the columns `age`, `gender`, `married`, `voted` and also `Q1` - `Q26`.
...
nerd_red <- nerd %>% 
  select(age, gender, married, voted, Q1:Q26)
```
...
```{r}
nerd_red <- nerd_red %>% 
  mutate(age_character = as.character(age), .before = gender)

nerd_red
```

3.1) By overwriting `nerd_red`, I permanently added the `age_character` column to the data frame. Can you turn it into a numeric column again? You can replace it using the old column name (i.e. `mutate(age_character = ...)`). (Of course, the name will be pretty misleading then.)

3.2) According to the code book, `gender` as been coded as 1 = Male, 2 = Female, 3 = Other. Assume there has been a mistake and it's actually 0 = Male, 1 = Female, 2 = Other. How can you correct the mistake? Don't overwrite the data frame, because we want to keep the old (correct) `gender` column as it is.
...
ifelse(some_numbers > 9, "more", "one")
```
...
```{r}
ifelse(some_numbers > 9, some_numbers *-1, some_numbers)
```
...
4.1) Recode the `married` column. It's 1 = Never married, 2 = Currently married, 3 = Previously married. Overwrite `nerd_red` with the version that contains the recoded column.

4.2) Recode the `voted` column. It's 1 = Yes, 2 = No. Overwrite `nerd_red` with the version that contains the recoded column.

4.3) Like `ifelse()`, `case_when()` can be used outside of `mutate()`. Can you use `case_when()` with the following vector so that you:
